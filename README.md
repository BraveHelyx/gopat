# Gopat
Gopat is a pattern generator and locator program similar to the `pattern_create` and `pattern_offset` ruby scripts in Kali. It will output a patterned string based off the `size` command line flag. By default it will output a 16 character patterned string.

It is also capable of locating a substring within a patterned string when invoked with the `find` command line flag.

## Installation
`go get github.com/BraveHelyx/gopat`

## Usage
### Pattern Creation
```
$ ./gopat --size=512

```

### Pattern Location
```
$ ./gopat --size=512 --find=Aq6A
Found substring at offset: 498
```
